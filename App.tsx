  ...
  <header className="bg-neutral shadow-md">
    <nav className="container mx-auto px-4 sm:px-6 md:px-8 py-3 flex justify-between items-center">
      ...
      <div className="flex items-center space-x-2 sm:space-x-4">
        <button
          onClick={() => setAdminView("DASHBOARD")}
          className="flex items-center space-x-2 text-gray-300 hover:text-white transition p-2 rounded-lg hover:bg-neutral/50"
        >
          <HomeIcon className="w-5 h-5" />{" "}
          <span className="hidden sm:inline">Dashboard</span>
        </button>
        <button
          onClick={() => setAdminView("ALL_CUSTOMERS")}
          className="flex items-center space-x-2 text-gray-300 hover:text-white transition p-2 rounded-lg hover:bg-neutral/50"
        >
        ...
        <button
          onClick={() => setAdminView("ADD_CUSTOMER")}
          className="flex items-center space-x-2 bg-primary hover:bg-secondary text-white font-semibold px-3 py-2 rounded-lg transition"
        >
        ...
        <button
          onClick={onLogout}
          className="flex items-center space-x-2 text-red-400 hover:text-white transition p-2 rounded-lg hover:bg-red-600/50"
        >
...
function App() {
  ...
  const handleAddCustomer = (
    customerData: Omit<Customer, "purchaseHistory">
  ) => {
  ...
  const handleUpdateCustomer = (
    updatedData: Omit<Customer, "purchaseHistory">,
    newPurchase: Purchase | null
  ) => {
    setCustomers(
      customers.map((c) => {
        if (c.id === updatedData.id) {
          const newHistory = newPurchase
            ? [...c.purchaseHistory, newPurchase]
            : c.purchaseHistory;
      ...
      case "ADD_CUSTOMER":
        return (
          <CustomerForm
            ...
            onCancel={() => setAdminView("DASHBOARD")}
          />
        );
      default:
        return (
  ...
  return (
    <div className="min-h-screen bg-base-100 font-sans">
      ...
      <main className="container mx-auto px-4 sm:px-6 md:px-8 py-4">
        {notification && (
          <div
            className={`fixed top-20 right-5 z-50 p-4 rounded-lg shadow-lg text-white ${
              notification.startsWith("Error") ? "bg-error" : "bg-success"
            }`}
          >
            {notification}
          </div>
